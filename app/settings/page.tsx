            ...
            {notifItems.map(n => (
              <div key={n.key} className="flex items-center justify-between py-3 border-b border-gray-50 dark:border-gray-800 last:border-0">
                ...
                <button
                  ...
                  className={`relative w-12 h-6 rounded-full transition-colors disabled:opacity-40 ${notifications[n.key as keyof typeof notifications] && notifPermission === "granted" ? "bg-blue-600" : "bg-gray-200"}`}
                >
                  <span className={`absolute top-1 w-4 h-4 bg-white rounded-full shadow transition-transform ${notifications[n.key as keyof typeof notifications] && notifPermission === "granted" ? "translate-x-7" : "translate-x-1"}`} />
                </button>
              </div>
            ))}
          </div>
