      ...
      <div className="max-w-3xl mx-auto py-10 px-6 flex flex-col gap-6">
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-6">
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-gray-800 dark:text-white">{lang === "ru" ? "Личные данные" : "Personal info"}</h2>
          ...
          <div className="flex flex-col gap-4">
            <div>
              ...
              {editMode ? <input value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-blue-400" /> : <p className="text-gray-800 dark:text-gray-200 font-medium">{user.name}</p>}
            </div>
            <div>
              <label className="text-sm text-gray-500 mb-1 block">{t("profile.phone")}</label>
              {editMode ? <input value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} className="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-blue-400" /> : <p className="text-gray-800 dark:text-gray-200 font-medium">{user.phone}</p>}
            </div>
            <div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-6">
          <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-6">{lang === "ru" ? "Изменить пароль" : "Change password"}</h2>
          <div className="flex flex-col gap-4">
            <div>
              <label className="text-sm text-gray-500 mb-1 block">{lang === "ru" ? "Текущий пароль" : "Current password"}</label>
              <input type="password" value={passwordForm.current} onChange={e => setPasswordForm({...passwordForm, current: e.target.value})} className="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-blue-400" />
            </div>
            <div>
              <label className="text-sm text-gray-500 mb-1 block">{t("auth.new_password")}</label>
              <input type="password" value={passwordForm.new} onChange={e => setPasswordForm({...passwordForm, new: e.target.value})} className="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-blue-400" />
            </div>
            <div>
              <label className="text-sm text-gray-500 mb-1 block">{lang === "ru" ? "Повторите новый пароль" : "Confirm new password"}</label>
              <input type="password" value={passwordForm.confirm} onChange={e => setPasswordForm({...passwordForm, confirm: e.target.value})} className="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-lg px-4 py-2 text-sm focus:outline-none focus:border-blue-400" />
            </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-sm p-6">
          <h2 className="text-xl font-bold text-gray-800 dark:text-white mb-4">{lang === "ru" ? "Быстрые действия" : "Quick actions"}</h2>
